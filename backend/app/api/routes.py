...
async def convert(image_id: str):
    ...
    image_bytes = base64.b64decode(data_dict["content"])
    image = Image.open(io.BytesIO(image_bytes))

    doc_url, gemini_path, converted_path = convert_to_3d(image, file_name, image_bytes)  
...
async def get_results(image_id: str):
    ...
    import os
    print(f"gemini_path exists: {os.path.exists(gemini_path)} -> {gemini_path}")
    print(f"converted_path exists: {os.path.exists(converted_path)} -> {converted_path}")
    print(f"doc_url: {data['doc_url']}")
